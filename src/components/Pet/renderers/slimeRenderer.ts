...
export const slimeRenderer: ProceduralRenderer = {
  getHaloColor: (mood: Mood) => {
    if (mood === 'dead') return 'rgba(156,163,175,0.15)'; 
    if (mood === 'sick') return 'rgba(163,230,53,0.15)'; 
    if (mood === 'sad') return 'rgba(59,130,246,0.1)'; 
  ...
  draw: ({ ctx, frame, mood, interactionProgress, interactionType, CANVAS_SIZE }: RenderContext): RenderResult => {
      ...
      switch (mood) {
        case 'happy':
          jumpOffset = Math.abs(Math.sin(frame * 0.04) * 8); // Rebond lent
        ...
        case 'sick':
          ...
          breath = Math.sin(frame * 0.05) * 0.8; // Fievre
          shapeSquash = 2;
          break;
        case 'sleeping':
          eyesStyle = 'closed';
      ...
      if (interactionProgress > 0) {
          ...
          if (interactionType === 0) {
              ...
              if (progress < 0.3) {
                  overrideSquash = progress * 20; 
                  overrideJump = - (progress * 5); 
              } else {
      ...
      // DESSIN DES YEUX
      ctx.fillStyle = `rgb(${targetPalette[1].slice(0,3).join(',')})`;
      const eyeSpacing = 8 + (breath * 0.5) + (overrideSquash * 0.3);
